...
//	printf("4、全灭\r\n"); 			
//	printf("5、状态\r\n"); 		
	nAsrStatus = msg;//LD_ASR_NONE;//初始状态：没有在作ASR
...
//（有声音输入，不论识别成功或失败都有中断）
	...
	{	 
			...
			if(nAsrResCount>0 && nAsrResCount<=4) 
			{
				...
				MSG_SEND_MSG(MSG_LD_IDENTIFY,LD_ASR_FOUNDOK);
			}
			else
			{
				//nAsrStatus=LD_ASR_FOUNDZERO;
				MSG_SEND_MSG(MSG_LD_IDENTIFY,LD_ASR_FOUNDZERO);
			}	
	}
	else
	{
